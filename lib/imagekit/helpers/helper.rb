...
module Imagekit
  class Helper
    ...
    def build_url(options)
      ...
      add_as_query = Imagekit::TransformationUtils.add_as_query_parameter?(opts) ||
      ...
      unless is_absolute_url
        ...
        if !transformation_string.empty? && !add_as_query
          path_parts << "#{TRANSFORMATION_PARAMETER}#{Imagekit::TransformationUtils.get_chain_transform_delimiter}#{transformation_placeholder}"
        end
    ...
    def build_transformation_string(transformations)
      ...
      transformations.each do |transform|
        ...
        current_transform.each do |key, value|
          ...
          transform_key = Imagekit::TransformationUtils.get_transform_key(key)
          ...
          parsed_transform_step << "#{transform_key}#{Imagekit::TransformationUtils.get_transform_key_value_delimiter}#{value}"
        end

        unless parsed_transform_step.empty?
          parsed_transforms << parsed_transform_step.join(Imagekit::TransformationUtils.get_transform_delimiter)
        end
      end

      parsed_transforms.join(Imagekit::TransformationUtils.get_chain_transform_delimiter)
    end
    ...
    def get_authentication_parameters_internal(token, expire, private_key)
      ...
      signature = Imagekit::CryptoUtils.create_hmac_sha1(private_key, token.to_s + expire.to_s)
    ...
    def get_signature(private_key:, url:, url_endpoint:, expiry_timestamp:)
      ...
      Imagekit::CryptoUtils.create_hmac_sha1(private_key, string_to_sign)
    end
